...
class SACAgent:
    ...
    def choose_action(self, states):
        ...
        # Convert continuous action to discrete action for Atari
        if self.config["n_actions"] <= 10:  # Assume discrete environment like Atari
            # Map continuous action to discrete action
            action_idx = int(np.argmax(action))  # Take the action with highest value
            return action_idx
        else:
            return action
    ...
    def store(self, state, z, done, action, next_state):
        ...
        # Convert discrete action to one-hot for Atari environments
        if self.config["n_actions"] <= 10:  # Assume discrete environment like Atari
            action_one_hot = np.zeros(self.config["n_actions"])
            action_one_hot[action] = 1.0
            action = torch.Tensor(action_one_hot).to("cpu")
        else:
